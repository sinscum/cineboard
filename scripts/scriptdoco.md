...
# CineBoard Script - Download and start install steps
## What is this?
If you implement the `Sleep Timer` feature in CineBoard, the screen is blanked out, but still powered on. This script uses the [`CEC`](https://support.google.com/chromecast/answer/7199917?hl=en#:~:text=CEC%20(Consumer%20Electronics%20Control)%20allows,Google%20streaming%20device%20is%20connected.) feature of your HDMI connected display to put the display in standby mode. This will save power and increase the longevity of your display.

*CineBoard scripts are designed to work on a rPi with a connected CEC compatible display.* 

## Compatibility
There are items required for the script 
- These scripts will only work on a `Raspberry Pi 4/5`, that is being used to display CineBoard
...
## Get the CineBoard scripts 
### Download
```
cd ~/
curl -sSL https://raw.githubusercontent.com/sinscum/cineboard/master/scripts/rpiwatcherservice.tar.gz -o rpiwatcherservice.tar.gz 
mkdir ~/cineboard-scripts
```
### Decompress
```
tar -xvf rpiwatcherservice.tar.gz -C ~/cineboard-scripts
``` 
### Set script permissions
```
cd ~/cineboard-scripts
...
```
~/cineboard-scripts/sleepwatch-remove-service.sh
cd ~
sudo rm -R cineboard-scripts
```
## Troubleshooting
...
- The sleepwatch script polls a CineBoard API ever 5 seconds to determine if CineBoard is in scheduled sleep mode or not. It then issues an `on` or `standby` command to the attached display.

The script does the following:
- Gathers the URL for CineBoard, current user and home directory
