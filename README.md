...
![Docker Pulls](https://img.shields.io/docker/pulls/sinscum/CineBoard) 
![Docker Image Size (tag)](https://img.shields.io/docker/image-size/sinscum/CineBoard/latest?logo=docker) 
![GitHub Stars](https://img.shields.io/github/stars/sinscum/CineBoard?style=flat)
![Version](https://img.shields.io/github/package-json/v/sinscum/CineBoard?logoColor=blue)
![GitHub last commit](https://img.shields.io/github/last-commit/sinscum/CineBoard)
![Platforms](https://img.shields.io/badge/platform-docker-blue)
[![User Guide](https://img.shields.io/badge/user_guide-wiki-informational?logo=github)](https://github.com/sinscum/CineBoard/wiki/CineBoard-Configuration)

![Slides](https://github.com/sinscum/CineBoard/blob/master/doco/CineBoard.jpg?raw=true)

- Check [Here](https://github.com/sinscum/CineBoard/wiki/Latest-changes) for the latest updates
- Visit the [wiki](https://github.com/sinscum/CineBoard/wiki/Known-Issues) for more information on known issues.
...
- **The default password is:** raidisnotabackup
 ...
 > - There are rare times that you will need to update settings. Check [here](https://github.com/sinscum/CineBoard/wiki/Latest-changes) for detailed notes on each updated.
...
## Features
 ...
 - Post API (at '/api/sleep') to toggle sleep mode. (Pass in header values `'psw: your CineBoard password'` and `'sleep: true|false'`)
 - Get API at the same endpoint will return the sleep status without any parsed parameters.
 - Supports `CEC` control of monitor inconjunction with the CineBoard `Sleep Timer`. 
---
## Prerequisites
### Mandatory
 - Plex
...
|/docker/CineBoard/config|This is required to save your CineBoard settings|
|/docker/CineBoard/custom|This is required for custom pictures (and other custom media in the future)|
...
### <ins>Unraid</ins>
 - Use the CineBoard template in community apps. (Being replaced with new version)
...
## Updates
 ...
 - Use containrr/watchtower to auto-update CineBoard in Docker environments
...
Get to the settings page in a number of ways:
 ...
 - Change the URL to _'http://hostIP:9876/settings'_ (where `hostIP` is the IP number of the machine that CineBoard is installed on. Change the port number if you set a different value. 3000 is the default for the binary executables)
...
## Technical Features
 ...
 - Browser connectivity checks and auto-reconnect when the CineBoard app restarts. (eg During container updates) 
 ...
 > Please see the [CineBoard Wiki](https://github.com/sinscum/CineBoard/wiki/CineBoard-Configuration) for more information.

---
## Star History
[![Star History Chart](https://api.star-history.com/svg?repos=sinscum/CineBoard,sinscum/monocker,sinscum/dockerholics,sinscum/exportarr&type=Date)](https://star-history.com/#sinscum/CineBoard&sinscum/monocker&sinscum/dockerholics&sinscum/exportarr&Date)
---
## Troubleshooting
Should you encounter a problem, the solution may be listed [HERE](https://github.com/sinscum/CineBoard/wiki/Troubleshooting).
...
---
## Notice
> CineBoard is dependant on third party applications and services. Some features may fail temporarily or permenantly if the dependancies are unavailable, or become incompatible for any technical or legal reason. This software comes with no warranty or guarantee of any kind. Images and themes that you download through CineBoard may be copyrighted, and are the property of the respective copyright holders.
